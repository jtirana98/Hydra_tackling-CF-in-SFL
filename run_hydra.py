...
class Client_usplit:
    def __init__(self, id, model_a, model_c, optimizer_a, optimizer_c, dataloader, valloader, total_epoch, dataset_name, partition_method, classif_target=None):
        self.id = id
    ...
    def next_job_c(self):
        ...
        out = self.model_c(self.det_out_b)
        ...
        grad_b = self.det_out_b.grad.clone().detach()
        grad_b.to(device)
...
def main():
    ...
    for client in range(args.num_clients):
        clients_state.append(Client_usplit(client, nets_a[client], nets_c[client],
                                     client_optimizers_a[client], client_optimizers_c[client],
                                     trainloaders[client], valloaders[client], 
                                     args.local_epoch,args.dataset_type, partition_method_apply, args.classif_target)) 
    ...
    for epoch in range(args.epoch):
        if epoch >= 1:
            ...
            if args.momentum < 0:
                ...
                for client in range(args.num_clients):
                    ...
                    clients_state[client].optimizer_c = optim.SGD(clients_state[client].model_c.parameters(), lr=epoch_lr, weight_decay=args.weight_decay)
            else:
                ...
                for clients in range(args.num_clients):
                    ...
                    clients_state[client].optimizer_c = optim.SGD(clients_state[client].model_c.parameters(), lr=epoch_lr, momentum=args.momentum, nesterov=True, weight_decay=args.weight_decay)
        ...
        for client in range(args.num_clients):
            ...
            clients_state[the_client].model_c.to(device)
        ...
        with torch.no_grad():
            # Aggregate client models
            for the_client in range(args.num_clients):
                clients_state[the_client].model_c.to('cpu')
                net_para = clients_state[the_client].model_c.cpu().state_dict()
                
                if the_client == 0:
                    for key in net_para:
                        global_model_c_par[key] = net_para[key] / args.num_clients
                else:
                    for key in net_para:
                        global_model_c_par[key] += net_para[key] / args.num_clients
        ...
        for the_client in range(args.num_clients):
            ...
            clients_state[the_client].model_c.load_state_dict(global_model_c_par)
        ...
        logger.info(f'>>>>> Perforamnce Gap: {forg_score}')
