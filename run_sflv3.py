    ...
    for epoch in range(args.epoch):
        ...
        with torch.no_grad():
            ...
            for the_client in range(args.num_clients):
                nets_b[the_client].to('cpu')
                net_para = nets_b[the_client].cpu().state_dict()
                
                if the_client == 0:
                    for key in net_para:
                        global_model_b_par[key] = net_para[key] / args.num_clients
                else:
                    for key in net_para:
                        global_model_b_par[key] += net_para[key] / args.num_clients
        global_model_b.load_state_dict(global_model_b_par)
        for the_client in range(args.num_clients):
            nets_b[the_client].load_state_dict(global_model_b_par)
        
        # Epoch ecaluation
        global_model_b_par = global_model_b.state_dict()
        global_model_bb.load_state_dict(global_model_b_par)

        forg_score, scores, epoch_accuracy, bw_score_val = my_utils.test_per_target_fl3(clients_state, global_model_bb, testloader, args.dataset_type,
